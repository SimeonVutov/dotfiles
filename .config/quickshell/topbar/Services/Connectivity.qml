...
Singleton {
    ...
    Timer {
        id: untrustedCleanup
        interval: 250
        onTriggered: root.removeUntrustedBluetooth()
    }
    ...
    Instantiator {
        ...
        delegate: Connections {
            ...
            target: modelData
            ignoreUnknownSignals: true
            property bool wasTrusted: modelData.trusted
            function onConnectedChanged() {
                root.stateRevision++;
            ...
            function onTrustedChanged() {
                if (wasTrusted && !modelData.trusted && modelData.paired)
                    root.forgetBluetooth(modelData);
                wasTrusted = modelData.trusted;
            }
        }
    }
    ...
    function actBluetooth(device, action) {
        if (bluetoothBusy || device.pairing || device.state === BluetoothDeviceState.Connecting || device.state === BluetoothDeviceState.Disconnecting)
            return;
        if (device.blocked) {
    ...
    function forgetBluetooth(device) {
        if (bluetoothBusy || !device.paired)
            return;
        error = "";
        device.forget();
    }
    function removeUntrustedBluetooth() {
